...
conn.level = global.db.data.users[m.sender]
  ...
  m.reply(`*Pet Kamu* (kucing,kuda,rubah ${global.db.data.users[m.sender].kucing,kuda,rubah}) menantang *${conn.getName(lawan)}* (kucing,kuda,naga,phonix,kyubi,centaur,griffin,serigala ${global.db.data.users[lawan].kucing,kuda,naga,phonix,kyubi,centaur,griffin,serigala) dan sedang dalam pertarungan sengit.\n\nTunggu ${lamaPertarungan} menit lagi dan lihat siapa yg menang.`)
  ...
  let kesempatan = []
  for (i=0;i<global.db.data.users[m.sender].kucing,kuda,rubah;i++) kesempatan.push(m.sender)
  for (i=0;i<global.db.data.users[lawan].kucing,kuda,rubah;i++) kesempatan.push(lawan)
