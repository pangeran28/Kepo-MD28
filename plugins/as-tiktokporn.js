...
🚀 *Link:* ${await(await axios.get(`https://tinyurl.com/api-create.php?url=${"https://tikporntok.com/"+video}`)).data}`
  ...
  if("https://tikporntok.com/"+video) conn.sendButton(m.chat, sel, wm, "https://tikporntok.com/"+video, [['Next', '.tiktokporn']], m, {jpegThumbnail: await(await fetch("https://tikporntok.com/"+thumb)).buffer()})
  else return conn.sendButton(m.chat, `Maaf media ${command} tidak ditemukan silahkan ulangi!`, wm, null, [["Repeat",  ".tiktokporn"]], m) 
 }
}
