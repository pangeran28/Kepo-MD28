let handler = async (m, { conn, usedPrefix, command }) => {
  ...
  conn.send2ButtonImg(m.chat, `Nihh @${m.sender.split('@')[0]} jangan sagne ya!`, wm, json.url, [[`Next`, `${usedPrefix + command}`]], m, {mentions: [m.sender], jpegThumbnail: await(await fetch(json.url)).buffer()})
  } catch {
